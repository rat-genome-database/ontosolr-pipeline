...
$APPHOME/run_sql $APPHOME/sql/export_genes.sql $ONTODATA/onto_solr_genes.tsv.unsorted
sort -ro $ONTODATA/onto_solr_genes.tsv $ONTODATA/onto_solr_genes.tsv.unsorted

echo "--EXPORTING HP terms as XP to .tsv file"
$APPHOME/run_sql $APPHOME/sql/export_HP_terms.sql $ONTODATA/HP_solr.tsv
sed 's/HP/XP/g' $ONTODATA/HP_solr.tsv > $ONTODATA/XP_solr.tsv

echo "--EXPORTING HP to RDO mappings"
$APPHOME/run_sql $APPHOME/sql/HP_and_RDO_mapped.sql $ONTODATA/HP_and_RDO_mapped.tsv
